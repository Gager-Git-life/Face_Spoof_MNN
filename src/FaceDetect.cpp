 ...
 * @version: 
 * @Author: Gager
 * @Date: 2020-12-08 15:57:04
 * @LastEditors: sueRimn
 * @LastEditTime: 2020-12-08 16:29:39
 */
...
FaceDetect::FaceDetect(const string& proto_model_dir, int num_thread_, float threhold_p_, float threhold_r_, float threhold_o_, float factor_){
    ...
    config.numThread = num_thread_; // 1 faster
...
uint8_t* get_img(cv::Mat img){
    ...
    cv::cvtColor(img, MatTemp, cv::COLOR_BGRA2RGBA, 4);
